...
<div class="container-xl px-2 mt-n10">
    <form action="{{ route('product-code.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="row">
					...
					<div class="col-md-6">
                                ...
                                <select class="form-select form-control-solid @error('vendor') is-invalid @enderror" id="vendor" name="vendor">
                                    ...
                                    @foreach($vendors as $vendor)
                                        <option @if(old('type') == $vendor->id )selected="selected"@endif value="{{ $vendor->id }}">{{ $vendor->company_name }}</option>
                                    @endforeach
                            ...
                            <input class="form-control form-control-solid @error('product_code') is-invalid @enderror" id="product_code" name="product_code" type="text" placeholder="" value="{{ old('product_code') }}" />
                            @error('product_code')
                            <div class="invalid-feedback">
                                {{ $message }}
                            </div>
                        ...
                        <div class="mb-3">
                                ...
                                <textarea class="form-control form-control-solid @error('description') is-invalid @enderror" id="description" name="description" rows="3">{{ old('description') }}</textarea>
                                @error('description')
                                <div class="invalid-feedback">
                                    {{ $message }}
                                </div>
